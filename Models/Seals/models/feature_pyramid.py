...
class FeaturePyramid(nn.Module):
    ...
    def __init__(self, backbone_layers, first=3, features=32,
        ...
        self.reduce = Parallel(
            named([make_reducer(size) for size in encoded_sizes]))
        self.decoder = UpCascade(
            named([make_decoder(features) for size in encoded_sizes]))
...
def extend_layers(layers, size, features=32):
    ...
    layers += [extra_layer(features_in if i == 0 else features, features)
