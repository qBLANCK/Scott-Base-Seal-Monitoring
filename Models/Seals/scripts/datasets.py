...
from libs.tools import struct, to_structs, concat_lists, to_dicts, pluck, pprint_struct, sum_list
import libs.tools
...
def annotation_summary(dataset):
    ...
    return struct(n_images=len(infos),
                  ...
                  size_ranges=(min(sizes, key=image_area),
                               max(sizes, key=image_area))
...
def match_datasets(dataset1, dataset2, threshold=0.5, check_overlap=True):
    ...
    for k, image1 in images1.items():
        ...
        results[k] = struct(count1=image1._size,
                            count2=image2._size, matched=len(matches))
...
def get_counts(dataset, class_id=None):
    ...
    def count(image):
        n = weighted_total(image, dataset.config) if class_id is None else len(
            class_annotations(image, class_id))
