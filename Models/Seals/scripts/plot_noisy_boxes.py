...
def add_noise(box, offset=0, noise=0):
    centre = box.centre + offset * box.size + \
        np.random.normal(0, noise, 2) * box.size
...
def average_iou(offset=0, noise=0, samples=100000):
    ...
    ious = [add_noise(box, offset / 100, noise / 100)[1]
...
def plot_boxes(ax):
    ...
    def add_box(box, color='g', linewidth=1):
        rect = patches.Rectangle(box.centre - box.size / 2, *box.size,
    ...
    for i in range(0, 5):
        for j in range(0, 5):
            ...
            box = struct(centre=np.array(
                [i+1, j+1]), size=np.array([0.5, 0.5]))
            add_box(box)

            for k in range(0, 10):
                noisy, _ = add_noise(box, offset=offset /
                                     100, noise=noise / 100)
                add_box(noisy, color=colors(k), linewidth=0.5)

            miou = average_iou(offset, noise)
            ax.text(box.centre[0], box.centre[1], "{:.1f}".format(
                miou * 100), horizontalalignment='center', verticalalignment='center')
