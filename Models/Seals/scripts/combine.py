...
from libs.tools import struct, to_structs, filter_none, drop_while, concat_lists, map_dict, \
...
def combine_penguins(datasets):
    val_royds = set_category_all(get_category(
        datasets.royds, 'validate'), 'test_royds')
    val_hallett = set_category_all(get_category(
        datasets.hallett, 'validate'), 'test_hallett')
    val_cotter = set_category_all(get_category(
        datasets.cotter, 'validate'), 'test_cotter')
...
def write_to(output, filename):
    ...
    with open(filename, 'w') as outfile:
        json.dump(to_dicts(output), outfile, sort_keys=True,
                  indent=4, separators=(',', ': '))
...
if __name__ == '__main__':
   ...
   # apples = load_all(apples_files, base_path)
    ...
    write_to(combine_simple(oliver), path.join(
        base_path, "oliver/combined.json"))
