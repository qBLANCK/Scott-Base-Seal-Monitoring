...
from libs.tools import to_structs, transpose_structs, transpose_dicts
...
def plot_sizes(loaded, keys, labels=dataset_labels):
    ...
    length_quartiles = {
        k: loaded[k].summary.box_length.quartiles for k in keys}
...
def plot_durations(loaded, keys, color_map=dataset_colors, labels=dataset_labels):
    durations_quartiles = {
        k: loaded[k].summary.image_durations.quartiles for k in keys}
...
def plot_instances_density(loaded, keys, color_map=dataset_colors, labels=dataset_labels):
    instances_quartiles = {
        k: loaded[k].summary.instances_image.quartiles + 1 for k in keys}
...
def plot_instances(loaded, keys, color_map=dataset_colors, labels=dataset_labels):
    ...
    for i, k in enumerate(keys):
        ...
        plt.scatter(instances + 1, np.repeat(i, instances.shape),
                    s=100, marker='|', color=color_map[k])
...
def plot_category_stacks(stacks, keys, color_map, categories):
    ...
    for i, k in enumerate(keys):
        ...
        p = ax.barh(np.arange(n) + 0.5, values, 0.5,
                    left=total, color=color_map[k])
        bars.append(p[0])
...
def plot_category_bars(stacks, keys, color_map, categories, cat_labels):
    ...
    for i, k in enumerate(keys):
        ...
        p = ax.bar(np.arange(n) - 0.4 + (i + 0.5) * width,
                   values, width, color=color_map[k])
        bars.append(p[0])

    plt.xticks(np.arange(n), [cat_labels[c]
               for c in categories], rotation='vertical')
    plt.legend(bars, keys)
    return fig, ax
...
def plot_times_density(loaded, keys, color_map, labels):
    ...
    for k in keys:
        ...
        times = [action.real_duration for action in sum_list(
            summaries).actions]
...
def export_summary_csvs(figure_path, summaries, labels):
    ...
    export_summary_table(
        path.join(figure_path, "data_summary.csv"), data_keys,  summaries, labels)

    annotation_keys = ['n_actions', 'n_annotations', 'total_minutes',
                       'instances_minute', 'actions_minute', 'actions_annotation']

    export_summary_table(path.join(
        figure_path, "time_summary.csv"), annotation_keys,  summaries, labels)

    export_summary_table(path.join(figure_path, "correction_summary.csv"),
                         correction_types, summaries._map(lambda s: s.correction_count), labels)
...
if __name__ == '__main__':
    ...
    fig, ax = plot_times_density(
        loaded, keys, color_map=dataset_colors, labels=dataset_labels)
    fig.savefig(path.join(figure_path, "time_density.pdf"),
                bbox_inches='tight')

    actions_proportions = summaries._map(
        lambda s: s.actions_count / s.total_actions)
    fig, ax = plot_category_bars(actions_proportions, action_types,
                                 color_map=action_colors, categories=keys, cat_labels=dataset_labels)
    ax.set_ylabel('proportion of actions')
    fig.set_size_inches(8, 7)
    fig.savefig(path.join(figure_path, "action_counts.pdf"),
                bbox_inches='tight')

    correction_types = ['weak positive', 'modified positive',
                        'false negative', 'false positive']

    correction_proportions = summaries._map(
        lambda s: s.correction_count / s.n_annotations)
    fig, ax = plot_category_bars(correction_proportions,  correction_types,
                                 color_map=correction_colors, categories=keys, cat_labels=dataset_labels)
    ax.set_ylabel('proportion of annotation count')

    ax.set_ylim(ymax=0.15)
    fig.set_size_inches(8, 7)
    fig.savefig(path.join(figure_path, "correction_counts.pdf"),
                bbox_inches='tight')

    fig, ax = plot_instances(loaded, keys=keys)
    fig.set_size_inches(16, 9)
    fig.savefig(path.join(figure_path, "instances_boxplot.pdf"),
                bbox_inches='tight')

    fig, ax = plot_durations(loaded, keys=keys)
    fig.set_size_inches(16, 9)
    fig.savefig(path.join(figure_path, "duration_boxplot.pdf"),
                bbox_inches='tight')

    fig, ax = plot_sizes(loaded, keys=keys)
    fig.set_size_inches(16, 9)
    fig.savefig(path.join(figure_path, "sizes_boxplot.pdf"),
                bbox_inches='tight')
