...
def actions_time_scatter(datasets, color_map, labels):
    ...
    for k in sorted(datasets.keys()):
        ...
        plt.scatter(actions, duration, s=instances * 5, alpha=0.5,
                    label=labels[k], color=color_map[k])
...
def instances_duration_scatter(datasets, keys, labels):
    ...
    for k in keys:
        ...
        plt.scatter(instances, duration, c=progress,
                    s=actions*5, label=labels[k], cmap='plasma')
...
def iou_lines(datasets, keys, color_map, labels):
    ...
    for k in keys:
        ...
        conf, iou, label = zip(
            *[point for point in get_annotation_ious(dataset) if point[1] < 1.0])
...
def confidence_iou_table(datasets):
    ...
    def compute(points):
        ...
        hist = np.histogram2d(np.array(conf), np.array(
            iou), bins=ranges)[0] / len(points)
...
def confidence_iou_scatter(datasets):
    ...
    hist, xbins, ybins, im = ax.hist2d(
        np.array(conf) - 0.01, np.array(iou) - 0.01, bins=(xrange, yrange), vmax=200)
...
if __name__ == '__main__':
    ...
    conf_subset = loaded._subset(
        'seals1', 'seals2', 'apples1', 'apples2', 'penguins', 'fisheye', 'branches')
    fig, ax = confidence_iou_scatter(conf_subset)
    fig.savefig(path.join(figure_path, "confidence_iou.pdf"),
                bbox_inches='tight')

    #t = confidence_iou_table(conf_subset)
    #export_csv(path.join(figure_path, "confidence_iou.csv"), [*conf_subset.keys(), 'total'], t.values())

    fig, ax = confidence_iou_scatter(
        loaded._subset('aerial_penguins', 'scott_base'))
    fig.savefig(path.join(figure_path, "confidence_iou_small.pdf"),
                bbox_inches='tight')

    fig, ax = iou_lines(loaded, keys=sorted(loaded.keys()),
                        color_map=dataset_colors, labels=dataset_labels)
    fig.savefig(path.join(figure_path, "iou_dataset.pdf"), bbox_inches='tight')

    fig, ax = actions_time_scatter(loaded._subset(
        'apples1', 'apples2'), color_map=dataset_colors, labels=dataset_labels)
    fig.savefig(path.join(figure_path, "apples_scatter.pdf"),
                bbox_inches='tight')

    fig, ax = actions_time_scatter(loaded._subset(
        'seals1', 'seals2'), color_map=dataset_colors, labels=dataset_labels)
    fig.savefig(path.join(figure_path, "seals_scatter.pdf"),
                bbox_inches='tight')
