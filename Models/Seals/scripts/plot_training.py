from os import path
from libs.tools import struct, to_structs, concat_lists, to_dicts, pluck, pprint_struct,  \
...
def get_prefixed(log, prefix):
    keys = [tag[len(prefix) + 1:]
            for tag in log.tags if tag.startswith(prefix)]
...
def plot_training_scatters(logs):
    ...
    for k, log in logs.items():
        ...
        if len(split):
            ...
            plt.scatter(
                AP_split, AP, label=dataset_labels[k], color=dataset_colors[k], marker='.')
...
def plot_training_lines(logs):
    ...
    for k, log in logs.items():
        ...
        if len(split):
            ...
            plt.plot(
                epoch, AP, label=dataset_labels[k], color=dataset_colors[k], linestyle='-')
            plt.plot(
                epoch, AP_split, label=dataset_labels[k], color=dataset_colors[k], linestyle='--')
...
def plot_noisy(run_path, figure_path):
    def read_run(offset, noise, dataset):
        logfile = path.join(log_path, run_path, str(
            noise), str(offset), dataset, 'log.json')
        return read_training(logfile)

    fig, ax = make_chart(size=(8, 8))
    datasets = ['penguins', 'scott_base',
                'seals', 'apples_lincoln', 'branches']
    ...
    for i, offset in enumerate(levels):
        for j, noise in enumerate(levels):
            logs = {dataset: read_run(offset, noise, dataset)
            ...
            ax.plot(epoch, AP, marker=markers[j], color=color_map(
                i / 4), linewidth=1, markersize=4)
    ...
    legend = [noise_level(i) for i in range(0, 5)] + \
        [offset_level(i) for i in range(0, 5)]
    ...
    with open(path.join(figure_path, run_path + ".csv"), mode='w') as csv_file:
        with open(path.join(figure_path, run_path + ".tex"), mode='w') as tex_file:
            ...
            for k, aps in {'COCO': APs, '30': AP30s, '50': AP50s, '75': AP75s}.items():
                ...
                tex_file.write(tex_table(k, np.mean(
                    combined, 0), np.std(combined, 0)))

    fig.savefig(path.join(figure_path, run_path +
                "_training.pdf"), bbox_inches='tight')


def plot_validation():

    fig, ax2d = plt.subplots(5, 2, sharex=True, sharey=True, figsize=(
        16, 26), gridspec_kw={'wspace': 0.1, 'hspace': 0.1})
    runs = struct(validate=struct(marker="x", style="-"),
                  validate_inc=struct(marker=".", style="--"))
...
def plot_lr(figure_path):
    ...
    def read_run(dataset, incremental, method, cycles, run):
        logfile = path.join(log_path, 'lr', run, incremental,
                            method, str(cycles), dataset, 'log.json')
    ...
    for dataset in datasets:
        ...
        for incremental in ["incremental", "full"]:
            for method in methods:
                ...
                for cycles in cycles_types:
                    ...
                    results = [
                        read_run(dataset, incremental, method, cycles, run) for run in runs]
                    if incremental == "full":
                        for r in results:
                            scatters[dataset] += list(zip(list(r.AP),
                                                      list(r.loss)))

                            aps[(method, cycles)] += list(r.AP[len(r.AP)//2:])
                            losses[(method, cycles)
                                   ] += list(r.loss[len(r.loss)//2:])

                    examples = results[0].examples
                    AP = sum([r.AP for r in results]) / len(results)
                    loss = sum([r.loss for r in results]) / len(results)

                    label = method if method is 'step' else method + \
                        "-" + str(cycles)
                    color = colors[label] if label in colors else tab10(
                        len(colors))
                    colors[label] = color

                    style = '--' if incremental == "incremental" else '-'

                    ax.plot(examples, AP, label=label,
                            linestyle=style, color=color)
                    ax2.plot(examples, loss, label=label,
                             linestyle=style, color=color)
        ...
        fig.savefig(path.join(figure_path, "lr_schedule",
                    dataset + ".pdf"), bbox_inches='tight')
    ...
    fig.savefig(path.join(figure_path, "lr_schedule",
                "scatter_loss_ap.pdf"), bbox_inches='tight')
...
def plot_multiclass(figure_path, directory, subsets):
    ...
    for subset, classes in subsets.items():
        ...
        for c in classes:
            ...
            plt.plot(epoch, ap_subset,
                     color=colors[c], linestyle='--', label=c)
    ...
    for dataset in datasets:
        ...
        for s, scale in enumerate(scales):
            for crop in crops:
                ...
                logfile = path.join(log_path, 'scales', str(
                    scale), str(crop), dataset, 'log.json')
                log = read_log(logfile)
                ...
                plt.plot(epoch, AP,  color=colors(s), linestyle=styles[crop], label=str(
                    1/scale * 100) + ":" + str(crop))
        ...
        fig.savefig(path.join(figure_path, "crops_scales",
                    dataset + ".pdf"), bbox_inches='tight')
...
def plot_schedules():
    ...
    plt.plot(times, list(map(cosine_anneal, times)),
             label="cosine annealing (SGDR)")
...
tab10 = plt.get_cmap("tab10")
pr_colors = {k: tab10(i) for i, k in enumerate(
    ["precision", "false positives", "false negatives", "true positives", "confidence"])}
...
def plot_pr_curves(ax, prs):
    ...
    for i, pr in prs.items():
        ...
        def suffix(s): return "$" + s + "_{" + str(i) + "}$"
        style = "--" if i == 50 else "-"

        # fpr = np.array(pr.false_positives)[1:] - np.array(pr.false_positives)[:-1]
        # print(fpr)

        l1 = ax.plot(recall, pr.precision, label="precision",
                     color=pr_colors["precision"], linestyle=style)
        l2 = ax2.plot(recall, pr.false_positives, label="false positives",
                      color=pr_colors["false positives"], linestyle=style)
        ...
        l5 = ax.plot(recall, pr.confidence, label="confidence",
                     color=pr_colors["confidence"], linestyle=style)
...
def plot_pr_grid(logs, keys, labels=dataset_labels):
    ...
    fig, axs = plt.subplots(
        1, 2, sharex=True, sharey=True, figsize=(16, rows*9))
    ...
    for k, ax in zip(keys, axs):
        ...
        APs = np.array(
            list(map(lambda entry: entry[1].AP, get_entry(log, "validate"))))
...
def export_best(figure_path):
    ...
    penguin_logs = read_logs(
        '/home/oliver/storage/logs/penguins', penguins_a._merge(penguins_b))

    best_validate = logs._map(best_epoch('AP'))
    best_penguin = penguin_logs._map(best_epoch('mAP50'))

    export_csv(path.join(figure_path, "validate.csv"), ap_keys,
               with_name(best_validate, dataset_labels))
    export_csv(path.join(figure_path, "validate_penguins.csv"),
               ap_keys, with_name(best_penguin, penguin_labels))
...
def seals_AP():
    ...
    logs = read_logs(path.join(log_path, 'validate'),
                     log_files._subset('seals1', 'seals2'))
