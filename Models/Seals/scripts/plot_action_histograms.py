...
from libs.tools import transpose_structs, struct
...
def uneven_gaussian_filter(x, y, w=1, dx=0.1, sigma=1):
    ...
    weights = w * np.exp(-delta_x*delta_x / (2*sigma*sigma)
...
def get_annotation_counts(dataset):
    counts = [image_summary(
        image).correction_count for image in dataset.history]
...
def plot_dataset_ratios(datasets, color_map, labels, sigma=5):
    ...
    for k, dataset in datasets.items():
        ...
        time, _ = get_time(dataset)
        x, y = uneven_gaussian_filter(
            time, ratios.positive, total, sigma=sigma)
...
def thresholds(image):
    changes = [
        action.value for action in image.actions if action.action == 'threshold']
...
def plot_action_histograms(loaded, color_map, labels, figure_path):
    ...
    fig.savefig(
        path.join(figure_path, "summaries/instance_rates.pdf"), bbox_inches='tight')

    fig, ax = plot_dataset_ratios(loaded, color_map, labels=labels, sigma=5)
    fig.savefig(
        path.join(figure_path, "summaries/positive_ratio.pdf"), bbox_inches='tight')

    for k, dataset in loaded.items():
        fig, ax = plot_combined_ratios(loaded[k], sigma=5)
        fig.savefig(path.join(figure_path, "action_annotations",
                    k + ".pdf"), bbox_inches='tight')

    fig, ax = cumulative_instances(loaded, color_map, labels=labels)
    fig.savefig(path.join(
        figure_path, "summaries/cumulative_instances.pdf"), bbox_inches='tight')

    ax.set_ylim(ymin=0, ymax=8000)
    ax.set_xlim(xmin=0, xmax=150)
    fig.savefig(path.join(
        figure_path, "summaries/cumulative_instances_crop.pdf"), bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    plot_action_histograms(loaded, color_map=dataset_colors,
                           labels=dataset_labels, figure_path=figure_path)
