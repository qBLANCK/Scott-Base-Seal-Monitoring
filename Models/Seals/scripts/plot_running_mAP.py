...
def plot_APs_dataset(r):
    ...
    for (i, iou) in enumerate(r.mAPs):
        plt.plot(r.times, r.mAPs[iou], color=colors(
            float(i)/len(r.mAPs)), label="$AP_{" + str(iou) + "}$")
...
if __name__ == '__main__':
    figure_path = "/home/fdi19/SENG402/Models/Seals/figures"
    ...
    fig, ax = plot_running_AP(
        results, color_map=dataset_colors, labels=dataset_labels)
