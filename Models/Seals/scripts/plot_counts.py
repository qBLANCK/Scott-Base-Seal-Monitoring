...
def sum_estimates(estimates):
    ...
    d = {k: sum([interp(e.times, e[k]) for e in estimates])
         for k in ['middle', 'lower', 'upper']}
...
def fill_gaps(images, delta):
    ...
    for i in range(len(images) - 1):
        ...
        while (next - t > min_gap):
            t += delta
            r.append(struct(time=t, category='discard',
                     estimate=images[i].estimate))
...
def get_estimates(images, delta=None):
    ...
    mask = torch.ByteTensor(
        [1 if i.category != 'discard' else 0 for i in images])
...
def plot_estimate(estimates, colour, style="-", show_estimates=True):
    ...
    if show_estimates:
        plt.fill_between(estimates.times, estimates.upper,
                         estimates.lower, facecolor=colour, alpha=0.4)
...
def plot_points(images, colour, marker, fill='none', key=lambda i: i.truth):
    ...
    plt.scatter(times, truth, marker=marker,
                edgecolors=colour, facecolors=fill)
...
def plot_point_sets(images, colour):
    ...
    plot_points(pick(images, ['discard']), colour,
                'o', key=lambda i: i.estimate.middle)
...
def plot_runs(*runs, loc='upper left', show_estimates=True, totals=None, delta=timedelta(minutes=10), size=(20, 10)):
    ...
    legend = list(map(run_legend, runs)) + ([run_legend(totals)] if totals is not None else []) + [
        ...
        Line2D([0], [0], marker='^', color='none',
               markeredgecolor='k', linestyle='None', label='train'),
        Line2D([0], [0], marker='s', color='none', markeredgecolor='k',
               linestyle='None', label='validate'),
        Line2D([0], [0], marker='o', color='none',
               markeredgecolor='k', linestyle='None', label='discard')
    ]
    ...
    for run, estimate in zip(runs, estimates):
        plot_estimate(estimate, colour=run.colour, style=run.get(
            'style', '-'), show_estimates=show_estimates)

    if totals is not None:
        total_estimates = sum_estimates(estimates)
        plot_estimate(total_estimates, colour=totals.colour, style=totals.get(
            'style', '-'), show_estimates=show_estimates)
...
def export_counts(file, counts):
    ...
    fields = ['image_file', 'time', 'truth',
              'category', 'lower', 'estimate', 'upper']
...
def plot_together(figure_path, loaded):
    ...
    images_100 = {
        image.image_file: image for image in scott_base_100 if image.category != 'new'}
    ...
    fig = plot_runs(
        struct(data=cam_b_100, colour='tab:olive',
               style="--", label="camera b (100)"),
        struct(data=cam_b, colour='forestgreen', label="camera b"),

        struct(data=cam_c_100, colour='skyblue',
               style="--", label="camera c (100)"),
        struct(data=cam_c, colour='royalblue', label="camera c"),
        show_estimates=False
    )

    fig.savefig(path.join(figure_path, "scott_base_combined.pdf"),
                bbox_inches='tight')
...
def plot_seals(figure_path, loaded):
    ...
    fig.savefig(path.join(figure_path, "seals_combined.pdf"),
                bbox_inches='tight')
