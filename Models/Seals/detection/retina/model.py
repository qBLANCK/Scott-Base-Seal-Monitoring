...
class Encoder:
    ...
    def anchors(self, input_size):
        ...
        if not (input_args in self.anchor_cache):
            ...
            layer_dims = [layer_size(self.start_layer + i)
                          for i in range(0, len(self.box_sizes))]
            anchors = anchor.make_anchors(
                self.box_sizes, layer_dims, device=self.device)
    ...
    def loss(self, input_size, target, encoding, prediction):
        ...
        if self.params.location_loss == "l1":
            loc_loss = loss.l1(encoding.location, location,
                               encoding.classification)
        elif self.params.location_loss == "giou":

            bbox = anchor.decode(
                location, anchor_boxes.unsqueeze(0).expand(location.size()))
...
class RetinaNet(nn.Module):
    ...
    def __init__(self, pyramid, num_boxes=9, num_classes=2, shared=False):
        ...
        def named(modules):
            modules = [(str(i + pyramid.first), module)
    ...
    def forward(self, input):
        ...
        return (
            torch.sigmoid(join_output(
                output.classification, self.num_classes)),
...
parameters = struct(
    ...
    shared=param(
        False, help="share weights between network heads at different levels"),
    ...
    params=group('parameters',
                 ...
                 balance=param(
                     4., help="loss = class_loss / balance + location loss")
                 ),
