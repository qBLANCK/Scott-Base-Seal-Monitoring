...
def draw_box(image, box, scale=1.0, name=None, confidence=None, thickness=2, color=(255, 0, 0), text_color=None):
    ...
    if not (name is None):
        image = cv.putText(image, name, (box[0], box[1] + width * 2), scale = 0.7 * scale, color=text_color, thickness=int(1*scale))

    if not (confidence is None):
        str = "{:.2f}".format(confidence)
        image = cv.putText(image, str, (box[0], box[3] - width * 2), scale = 0.7 * scale, color=text_color, thickness=int(1*scale))
