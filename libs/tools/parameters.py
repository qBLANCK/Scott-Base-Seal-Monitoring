...
def default_parameters(parameters):
    ...
    for name, param in parameters.items():
        if param.type == 'choice':
            defaults[name] = struct(choice=param.default, parameters=default_parameters(
...
def get_choice(_choice):
    return _choice.choice, _choice.parameters
    ...
    for name, parameter in parameters.items():
        if parameter.type == 'group':
            ...
            if (parameter.type == 'bool' and default is False):
                ...
                parser.add_argument('--' + name, required=parameter.required,
                                    default=default, type=to_type(parameter.type), help=help)
    return parser
