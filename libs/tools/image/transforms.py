...
def adjust_gamma(adjustment=0.1, per_channel=0):
    def f(image):
        if per_channel > 0:
            for d in range(0, 3):
                image.select(2, d).copy_(random_gamma(
                    image.select(2, d), per_channel))
...
def random_crop(image_size, crop_size, border_bias=0):
    ...
    def random_interval(lower, upper, size, border_bias=0):
        ...
        if size < max_size:
            pos = random.uniform(lower - border_bias,
                                 upper - size + border_bias)
...
def random_affine(image_size, dest_size, translation=0, scale_range=(1, 1), rotation_size=0, flip=False):
    ...
    tx, ty, s, r, flip = randoms(
        translation, translation, scale_range, rotation_size, 1 if flip else 0)
