...
def mouse_move(browser, sleep=0.1, absolute=False, x=None, y=None):
    ...
    # Find view size
    view_width = safe_integer(browser.execute_script(
        'return window.innerWidth'), 200)
    view_height = safe_integer(browser.execute_script(
        'return window.innerHeight'), 200)

    # Find view offset
    view_x = safe_integer(browser.execute_script(
        'return window.pageXOffset'), 0)
    view_y = safe_integer(browser.execute_script(
